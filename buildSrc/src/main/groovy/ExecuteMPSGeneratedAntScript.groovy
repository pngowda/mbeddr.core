...
class ExecuteMPSGeneratedAntScript extends DefaultTask {
    ...
    /**
    * This method reads inputs from *_incrementalIO.xml file generated by MPS build script generator and resolves the
    * path of each input files and returns as list and marks all of them InputFiles for the Gradle to work incrementally.
    * This works only if incremental build is enabled with the command line option "-Pincremental" and corresponding *_IO.xml file
    * exist for each generated ant build file. Otherwise it just creates a temporary file and marks it as input which is essential
    * to run build in the rebuild mode. (Note: temp file creation is just a workaround for time being)
    ...
    */
    ...
    /**
    * This method reads output from *_incrementalIO.xml file generated by MPS build script generator and resolves the
    * path of each input files and returns as list and marks all of them InputFiles for the Gradle to work incrementally.
    * This works only if incremental build is enabled with the command line option "-Pincremental" and corresponding *_IO.xml file
    * exist for each generated ant build file. Otherwise it just creates a temporary file and marks it as input which is essential
    * to run build in the rebuild mode. (Note: temp file creation is just a workaround for time being)
    ...
    */
    ...
    /**
    *Method which spawns an ant process to run the build with corresponding command line arguments,
    * ant build xml and targets to be executed for each ant script.
    */
    def spawnAnt() {
        project.javaexec {
